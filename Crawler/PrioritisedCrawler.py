...
logging.basicConfig(level=logging.INFO, filename='prioritised_crawler.log', filemode='w',
                    format="%(message)s")
...
class MyCrawler:
    ...
    def build_frontier(self):
        ...
        for url in self.seed_url:
            ...
            if key not in self.site_crawled:
                self.site_crawled[key] = (sys.maxsize, 1)
    ...
    def crawl(self):
        while self.crawl_count < MAX_CRAWL:
            ...
            # if priority has changed push back to queue
            if task.novelty < self.site_crawled[key][0]:
               self.frontier.put(task)
               continue

            # update crawl time of task
            ...
            with self.lock1:
                ...
                if self.crawl_count < MAX_CRAWL:
                    self.crawl_count += 1
                    self.site_crawled[key] = (self.site_crawled[key][0],
                                              self.site_crawled[key][1] + 1)
                else:
    ...
    def fetch(self, task):
        ...
        for link in parser.get_links():
            ...
            # check if the url is already in frontier queue
            if new_task.taskid in self.frontier:
                # increase importance score by 1
                new_task.importance += 1

            # check if the url belongs to a crawled site
            if key in self.site_crawled:
                # check if crawl limit exceeded for the site
                if self.site_crawled[key][1] > MAX_CRAWL_PER_SITE:
                    continue
                # decrease novelty score by 1
                with self.lock1:
                    self.site_crawled[key] = (self.site_crawled[key][0] - 1,
                                              self.site_crawled[key][1])
                new_task.novelty = self.site_crawled[key][0]
            else:
                with self.lock1:
                    self.site_crawled[key] = (sys.maxsize, 1)
...
def main():
    ...
    while mc.crawl_count < MAX_CRAWL:
        # call crawl
        for i in range(min((NUM_THREADS - threading.active_count()), mc.frontier.qsize())):
    ...
    print("Total crawl time:" + diff.__str__() + " Total crawled:" + mc.crawl_count.__str__() +
          ...
          " Total_size:" + mc.total_size.__str__() + "\n")
